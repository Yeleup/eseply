...
Печать одной квитанции: один лист A5 содержит один или два экземпляра (`copies_per_page`). Массовая печать раскладывает экземпляры на A4 сеткой 2×4 (до 8 экземпляров на листе, масштаб 0.7), поэтому содержимое экземпляра должно помещаться в половину листа A5. CSS шаблона вставляется на печатную страницу один раз. Файлы логотипа и QR удаляются при сбросе шаблона, замене файла и удалении организации.
