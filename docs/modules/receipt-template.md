...
Поля `logo_path`/`qr_path` приходят из публичного Livewire-состояния формы, поэтому перед сохранением сервер отдельно проверяет (`App\Support\ReceiptTemplateImageStorage::belongsToOrganization`, вызывается из `ReceiptTemplatePage::save()`, плюс `preventFilePathTampering()` на самих полях FileUpload), что путь либо пуст, либо начинается с каталога `receipt-templates/{organization_id}/` текущей организации — подмена на путь другой организации или другого модуля (например, фото показаний) отклоняется, запись не сохраняется и файл не удаляется.
