...
- `make test` creates the worktree databases if needed (for example after `make down-volumes` in the main checkout), so parallel worktrees never share a test database;
